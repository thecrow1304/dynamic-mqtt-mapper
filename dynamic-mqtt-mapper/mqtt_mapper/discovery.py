...
def detect_component_and_config(key: str, val: dict):
    # Determine component and additional config
    ...
    if 'valueNumber' in val:
        ...
        if 'temperature' in k:
            ...
            # No dedicated device_class for volume; leave out
            pass
        return 'sensor', cfg
...
def publish_discovery(device_id: str, key: str, val: dict, meta: dict):
    ...
    obj_id = f"{slugify(device_id)}_{slugify(key)}"
...
def publish_state(device_id: str, key: str, val: dict):
    ...
    if 'valueBoolean' in val:
        ...
        # Prefer RFC3339 string if present
        iso = val.get('value')
