...
def ensure_device_meta(payload: dict):
    ...
    if device_id not in devices:
        ...
        # Update known meta fields if changed
...
def handle_mqtt_message(topic: str, payload: dict, mqtt_client=None):
    ...
    # iterate all keys in message
    for key, val in message.items():
        # Vorsicht: jeder val ist ein dict mit value*, informationType etc.
        # Speichere letzten Wert + Erkennung
        entity_rec = device['entities'].get(key, {})
        entity_rec['raw'] = val
        entity_rec['updated'] = datetime.utcnow().isoformat() + 'Z'

        # Speichere einfachen Wert für UI
        if 'valueBoolean' in val:
            ...
            entity_rec['type'] = 'sensor(timestam p)'
        ...
        # Discovery (idempotent, retained). Wird immer wieder gesendet – ok.
        try:
